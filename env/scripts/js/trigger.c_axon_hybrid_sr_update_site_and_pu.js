...
const { accounts, c_public_users, c_steps } = org.objects

const createdObj = script.arguments.new
const studyId = createdObj.c_study && createdObj.c_study._id
let publicUser = createdObj.c_public_user && c_public_users.readOne({ _id: createdObj.c_public_user._id })
  .skipAcl()
  .grant(consts.accessLevels.read)
  .execute()
const account = createdObj.c_account && accounts.find({ _id: createdObj.c_account._id })
  .skipAcl()
  .grant(consts.accessLevels.read)
  .expand('c_public_users')
  .next()
const accountId = (account && account._id) || (publicUser && publicUser.c_account && publicUser.c_account._id)
const updateOwner = !!(accountId && !script.principal._id.equals(accountId))
...
// active tasks don't have steps
if (!createdObj.c_step) return

const { c_personal_data } = c_steps.find({ _id: createdObj.c_step._id })
  .skipAcl()
  .grant('read')
  .next()

if (c_personal_data) {
  script.arguments.new.update({ c_personal_data: c_personal_data })
}